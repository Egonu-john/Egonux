...
import EvidenceReviewPanel from '@/components/anosa/EvidenceReviewPanel';
...
interface EvidenceStatus {
  ...
  signatureMode: 'cloud-kms' | 'unsigned';
}
...
export default function AnosaMobile({ principal, previewMode }: AnosaMobileProps) {
  ...
  const testPermanentEvidence = async () => {
    ...
    try {
      ...
      setNotice(`Cloud evidence verified. Canary ${body.canary.contentHash.slice(0, 12)} is permanently recorded${body.canary.signatureVerified ? ' with a verified Cloud KMS signature' : ''}.`);
    ...
    <section className={styles.phone} aria-label="ANOSA Personal Mobile">
      ...
      <div className={styles.content}>
        ...
        {activeTab === 'control' ? <div className={styles.view}>
          <div className={styles.viewTitle}><span>PHASE 3.4 · KMS EVIDENCE SIGNING</span><h1>Intent control center</h1><p>New permanent receipts can be signed by a Google Cloud KMS key and checked against their server audit events. External side effects remain disabled.</p></div>
          ...
          <div className={styles.boundaryList}><div><Icon name="check" /><span><strong>Policy engine</strong><small>Five mandatory checks per intent</small></span><em>Active</em></div><div><Icon name={evidenceStatus?.mode === 'cloud' ? 'check' : 'lock'} /><span><strong>Permanent evidence</strong><small>{evidenceStatus?.mode === 'cloud' ? 'Firestore append-only ledger' : 'Device ledger until cloud identity passes'}</small></span><em className={evidenceStatus?.mode === 'cloud' ? undefined : styles.locked}>{evidenceStatus?.mode === 'cloud' ? 'Cloud' : 'Gated'}</em></div><div><Icon name={evidenceStatus?.signingReady ? 'check' : 'lock'} /><span><strong>Cloud KMS signatures</strong><small>Google-managed · private key non-exportable</small></span><em className={evidenceStatus?.signingReady ? undefined : styles.locked}>{evidenceStatus?.signingReady ? 'Ready' : 'Gated'}</em></div><div><Icon name="lock" /><span><strong>Connectors</strong><small>Email, tasks, GitHub, payments, and production</small></span><em className={styles.locked}>Isolated</em></div><div><Icon name="lock" /><span><strong>External effects</strong><small>No send, publish, transfer, deploy, or access mutation</small></span><em className={styles.locked}>Disabled</em></div></div>
          ...
          <section className={styles.verificationCard} data-status={verification?.status ?? 'unavailable'}><div><Icon name={verification?.status === 'verified' ? 'check' : 'security'} size={20} /><span><small>LEDGER VERIFICATION</small><strong>{verification?.status === 'verified' ? 'Evidence chain verified' : verification?.status === 'attention' ? 'Audit review required' : 'Verification pending'}</strong></span></div><p>{verification ? `${verification.verifiedRecords} of ${verification.checkedRecords} records verified · ${verification.signedRecords} KMS signed · ${verification.brokenRecords} broken links` : 'Run verification to compare permanent receipts with their audit events.'}</p><em>EXECUTION DISABLED</em></section>
          <EvidenceReviewPanel verification={verification} />
          {verification?.auditEvents.length ? <section className={styles.history} aria-label="Audit trail"><div className={styles.sectionHeading}><div><span>SERVER AUDIT</span><h2>Recent audit trail</h2></div><small>{verification.auditEvents.length} checked</small></div>{verification.auditEvents.slice(0, 10).map((event) => <article key={event.id}><Icon name={event.verified ? 'check' : 'lock'} size={15} /><div><strong>{event.type.replace('anosa.', '').replace(/\./g, ' ')}</strong><small>v{event.schemaVersion}{event.signatureVerified ? ' · KMS signed' : ' · legacy hash'} · {event.contentHash.slice(0, 10)} · {new Date(event.occurredAt).toLocaleString()}</small></div><em>{event.verified ? 'verified' : 'review'}</em></article>)}</section> : null}
